...
async def benchmark_stack(name: str, base_url: str, num_requests: int = 100):
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        for i in range(num_requests):
            ...
            try:
                response = await client.post(f"{base_url}/api/shorten", json={"url": f"https://example.com/test/{i}"})
        ...
        return {
            ...
            "p95_latency_ms": (
                statistics.quantiles(latencies, n=20)[18] * 1000 if latencies and len(latencies) > 20 else 0
            ),
            "p99_latency_ms": (
                statistics.quantiles(latencies, n=100)[98] * 1000 if latencies and len(latencies) > 100 else 0
            ),
        }
...
async def main():
    ...
    python_results = await benchmark_stack(
        "Python (Direct)", "http://localhost:8000", 200  # This will be updated based on stack
    )

    # Test Rust stack (if available)
    try:
        rust_results = await benchmark_stack(
            "Rust (Direct)", "http://localhost:8000", 200  # This will be updated based on stack
        )
    except Exception as e:
    ...
    if python_results and rust_results:
        ...
        latency_improvement = (
            python_results["avg_latency_ms"] / rust_results["avg_latency_ms"]
            if rust_results["avg_latency_ms"] > 0
            else 0
        )
