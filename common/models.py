...
from apps.url_shortener.database import Base
...
class URL(Base):
    ...
    clicks: Mapped[int] = mapped_column(Integer, default=0, nullable=False, index=True)  # Added index
    created_at: Mapped[datetime.datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False, index=True  # Added index
    )
    ...
    # Composite index for hybrid sorting (clicks + recency)
    __table_args__ = (Index("idx_url_popularity", "clicks", "created_at", "updated_at"),)
