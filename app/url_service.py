...
KAFKA_EVENTS_FAILED_TOTAL = Counter(
    "url_shortener_kafka_events_failed_total", "Total Kafka events that failed to publish"
)
...
class URLShorteningService:
    ...
    async def _increment_click_buffer(self, short_code: str) -> None:
        ...
        if buffered_count == 1:
            await self._cache_write.expire(buffer_key, self._settings.CLICK_BUFFER_TTL_SECONDS)
            REDIS_OPERATIONS_TOTAL.inc()
